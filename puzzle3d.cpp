...
namespace {
    ...
    int32_t getIndex(const Point& p) { return p.z * N * N + p.y * N + p.x; }
    ...
    void printPatternVariations() {
        for (uint32_t i = 0; i < PATTERN_VARIATIONS.size(); ++i) {
            ...
            for (uint32_t j = 0; j < patternVariation.size(); ++j) {
                ...
                std::cout << std::format("printPatternVariations: patternPointIndex: {}, point: {}", j, patternPoint) << std::endl;
            }
            std::cout << std::endl;
        }
    }
    void generatePatternVariations() {
        ...
        for (uint32_t permutationIndex = 0; permutationIndex < 48; ++permutationIndex) {
            ...
            for (uint32_t i = 0; i < PATTERN.size(); ++i) {
                ...
                // std::cout << std::format("generatePatternVariations: permutationIndex: {}, indexInPattern: {}, coords: {} {} {}", permutationIndex, i, patternVariation.back()[0], patternVariation.back()[1], patternVariation.back()[2]) << std::endl;
            }
            std::sort(patternVariation.begin(), patternVariation.end());
            bool bIsUnique = true;
            for (uint32_t j = 0; j < PATTERN_VARIATIONS.size(); ++j) {
                // std::cout << std::format("testPermutationsUniqueness: permutationIndex: {}, j: {}", permutationIndex, j) << std::endl;
                if (std::equal(patternVariation.begin(), patternVariation.end(), PATTERN_VARIATIONS[j].begin())) {
                    // std::cout << std::format("testPermutationsUniqueness: permutationIndex: {} - not unique", permutationIndex) << std::endl;
                    bIsUnique = false;
                    break;
                }
            }
            if (bIsUnique) {
                // std::cout << std::format("testPermutationsUniqueness: permutationIndex: {} - unique", permutationIndex) << std::endl;
    ...
    {
        // std::cout << std::format("tryPlaceGroup: patternVariationIndex: {}, positionInPattern: {}, positionInCube: {}", patternVariationIndex, positionInPattern, positionInCube) << std::endl;
        ...
        for (uint32_t i = 0; i < PATTERN.size(); ++i) {
            ...
            // std::cout << std::format("tryPlaceGroup: positionInGroup: {}, {}, indexInCube: {}", i, p, indexInCube) << std::endl;
            if (!isInBounds(p) || CUBE[indexInCube] != UNDEFINED) {
                // std::cout << std::format("tryPlaceGroup: patternVariationIndex: {} - bad, return", patternVariationIndex) << std::endl;
                return false;
            }
    ...
    bool iterateGroup(const int32_t groupId) {
        if (groupId > DEEPEST_ITERATION) {
            ...
            std::cout << std::format("iterateGroup: deepest iteration so far: {}, probes: {}", DEEPEST_ITERATION, PROBES_TRIED) << std::endl;
        }
        ...
        for (int32_t posInCube = 0; posInCube < N * N * N; ++posInCube) {
            ...
            for (uint32_t patternVariationIndex = 0; patternVariationIndex < PATTERN_VARIATIONS.size(); ++patternVariationIndex)
            {
                // std::cout << std::format("iterateGroup: groupId: {}, patternVariationIndex: {}", groupId, patternVariationIndex) << std::endl;
                for (uint32_t posInPattern = 0; posInPattern < PATTERN.size(); ++posInPattern)
                {
                    // std::cout << std::format("iterateGroup: groupId: {}, patternVariationIndex: {}, posInPattern: {}", groupId, patternVariationIndex, posInPattern) << std::endl;
